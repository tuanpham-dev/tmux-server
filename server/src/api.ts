...
// ---- Host statistics ----
//
// Served as two readings, because the status bar polls this every 3 seconds
...
let lastNetRates: { rxBytesPerSec: number; txBytesPerSec: number } | null = null;

async function readNetworkRates(): Promise<{ rxBytesPerSec: number; txBytesPerSec: number } | null> {
  let rx = 0;
  let tx = 0;
  try {
...
// two mistakes.
async function readDisks(): Promise<
  { path: string; totalBytes: number; usedBytes: number; availableBytes: number }[]
> {
  const disks = [];
...
// Host stats for the status bar: memory pressure on the bar itself, and with
// ?detail=1 everything the popover that reading opens shows beside it (see
// the readers above for why that is a separate reading). The port count
...
api.get("/system-stats", async (req, res) => {
  try {
