...
- **Zero-lag local echo** — on mobile, typing into a pane whose foreground command matches a filter (default: `claude`) renders instantly in an overlay and buffers until Enter instead of round-tripping every keystroke through the server; in-progress IME/predictive-keyboard words show underlined before they commit. Configure or disable via Settings → Terminal's "Local echo when" filter.
