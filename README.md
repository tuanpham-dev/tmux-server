...
- **Worktree sessions** — the **WORKTREES** panel lists the active repo's git worktrees, each with its dirty state and the tmux session working in it. Create one from a new or existing branch (also from the command palette's "Worktrees: New Worktree Session…") and it's checked out and opened as its own session in one step — the clean way to run several agents or branches side by side without them colliding in one checkout. Removing a worktree is confirm-gated, offers to kill its session with it, asks again before discarding uncommitted changes, and always keeps the branch. Worktrees land in `.worktrees/<branch>` by default (configurable in Settings → Worktrees; kept out of `git status` via `.git/info/exclude`, never your committed `.gitignore`). Bundled as the `worktrees` extension.
...
- `ctx.app.openSessionWindow(sessionName, opts?: { createCwd? })` — opens a tmux session's active window as a window-tab; with `createCwd`, a session that doesn't exist yet is created there first (how the worktrees extension turns a fresh checkout into a session in one step)
