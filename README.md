...
- `ctx.registerSidebarPanel({ id, title, component })` — joins the sidebar accordion alongside SESSIONS/FILES/PORTS; the component receives one optional host prop, `actionsTarget` (a `HTMLDivElement | null` to portal header-row buttons into, e.g. a refresh/sync icon next to the panel's title — same portal mechanism `registerFileViewer`'s `toolbarTarget` uses)
