...
A VSCode-style web UI for tmux. Open folders as projects — each backed by its own tmux session — browse them (and every other tmux session) in a sidebar, open them as tabbed terminals, and manage sessions/windows — all from the browser over WebSockets, rendered by [xterm.js](https://xtermjs.org/) — or the optional [ghostty-web](https://www.npmjs.com/package/ghostty-web) (Ghostty's WASM terminal core) engine, installable from the extensions registry — selectable in Settings.
...
- **Terminal niceties** — **right-click** opens a context menu: Open Link / Open File / Preview / Copy Link or Path over a URL, file path, or OSC 8 hyperlink, plus Copy, Copy as Paragraph, Paste, Select All, and Clear anywhere (`Shift+F10` or the Menu key opens it at the cursor). Hold `Shift` while right-clicking to send the click to a program that's using the mouse (vim, htop) instead — or swap that around, or make right-click paste, in Settings → Terminal. `Ctrl+click` (`Cmd+click` on Mac) opens URLs, local file paths (with `:line[:col]` jumping), and hyperlinked text; scrollback search (`Ctrl+Shift+F`); Shift+drag for browser text selection instead of tmux copy-mode. **Paste or drop an image** onto a terminal and it's uploaded (to `/tmp` by default, configurable in Settings → Behavior, or `<cwd>/uploads` when that setting is cleared) with the saved path typed at the prompt — made for handing screenshots to Claude Code; ordinary text paste is untouched.
...
- **Status bar** — an optional strip along the bottom (Settings → UI) showing the active directory's git branch (with a `*` when it's dirty and the ahead/behind counts beside it — click to open SOURCE CONTROL), memory in use, plus counts of open terminals and listening ports. Clicking the terminal count opens the live terminals list, with the same row actions the PROJECTS panel has; clicking the port count lists every listening port with its forward-all command. Drag any readout to reorder it or move it between the bar's two ends. Extensions can add their own (`ctx.registerStatusBarItem`) — the bundled Claude usage extension puts its 5-hour and weekly limit meters here. On phones the bar stays, compact: every item drops to its icon and number, and the row scrolls if it has to.
- **Shell integration** — an optional, generated rc snippet (`~/.config/tmux-server/shell-integration.sh`; the exact source line to add to `~/.zshrc`/`~/.bashrc` is shown with a Copy button in Settings → Behavior) that marks every prompt with OSC 133 — which tmux ≥ 3.4 tracks natively — and reports each command's start, end, exit code, and duration to the server. It powers **jump to previous/next command** in the scrollback, per-pane **command history** with exit codes and durations, and **finished-command notifications**: set a threshold in Settings → Behavior and any command running at least that long triggers a web-push notification (with the command, exit status, and duration) through the same pipeline as bell notifications. Reports are fire-and-forget background requests that never block the prompt, and everything degrades to a no-op when the snippet isn't sourced.
...
- **AI, configured once** — Settings → AI holds every AI you want available: a CLI you're already signed into (`claude`, `codex`, `agy`), a keyed API (Anthropic's or OpenAI's wire format, pointed at the vendor or at any compatible endpoint — OpenRouter, Groq, Together, LiteLLM, a local Ollama), or any custom command that prints a reply. Add as many as you like and mark one the default; API keys are stored server-side and never sent back to a browser. A CLI that isn't installed on the server is greyed out rather than offered, and the **Model** field fetches whatever the provider will tell it: the endpoint's own `/models` list for the API providers (Anthropic's or OpenAI's, against whatever base URL you set — so a compatible service lists *its* models), `agy models` for Antigravity, and the aliases `claude` documents in its own `--help`. It still accepts any id you type, since a list is never the whole truth. Everything that asks for an AI — the commit-message button in SOURCE CONTROL, AI Command Search, the prompt editor's Refine — goes through that list, and an extension can point its own feature at a different entry — and at a different model on it (a cheaper, faster one for short jobs) — from its own settings section.
- **Agents, defined once** - Settings → Agents is the app's one list of AI agents, next to AI Providers. Each entry carries the three facts that always travel together: the foreground command tmux reports for a pane running it (that's how the app knows which window your agent is in - SOURCE CONTROL's "Send to Agent", the element pickers in LIVE PREVIEW and PORTS, AGENT MONITOR), the full launch line (what "Start work" offers when you open a branch or a ticket in a fresh worktree), and which hook format its CLI reads. It ships knowing Claude Code, Claude Code with permissions skipped, Codex and Antigravity; add your own wrapper script with an Add. Extensions read this list instead of each carrying a copy, so the same CLI is named once rather than in every extension that cares.
...
- **Copy modes** (Settings → Terminal → *When copying a selection*) — *Join soft-wrapped lines* (the default) undoes the wraps the terminal itself made, which tmux's redraws would otherwise turn into a newline at every wrap column, while keeping newlines the program actually emitted. *Keep lines as shown* copies the rows verbatim. *Join into paragraphs* also undoes a program's own word-wrap, so a paragraph Claude Code or `git log` wrapped for the pane width comes back as one paragraph — that one is lossy (a code block collapses onto one line), so prefer **Copy as Paragraph** (`Ctrl+Shift+Alt+C`, or the terminal's right-click menu) to apply it to a single copy without changing the default.
...
- `ctx.registerStatusBarItem({ id, placement?, order?, component })` — a compact readout in the bottom status bar. `placement` is `"left"` or `"right"` (default `"right"`, where the app's own memory and terminal readouts also sit) and `order` sorts within that group. The component receives a `context` prop with `mobilePointer`, `showMenu(x, y, items)`, and `openPopover(anchorRect, content)` / `closePopover()` — a floating panel above the bar that the host positions and dismisses, and that `openPopover` toggles when the same item calls it again (how the bundled ports extension shows its port list and forward command, and how the external Claude usage extension shows its token counter). The whole bar is hidden on phones and tablets, so an item never renders there
...
- `ctx.app.openSessionWindow(sessionName, opts?: { createCwd? })` — opens a tmux session's active window as a window-tab; with `createCwd`, a session that doesn't exist yet is created there first (how a fresh checkout becomes a session in one step)
- `ctx.app.killSession(sessionName)` — kills a session and closes its tabs, including the synthetic per-window attachments a raw `tmux kill-session` would orphan. Runs **no** confirmation of its own — the caller owns the prompt, so confirm before calling
