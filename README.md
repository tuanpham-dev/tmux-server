...
- **Terminal niceties** — `Ctrl+click` (`Cmd+click` on Mac) opens URLs, local file paths (with `:line[:col]` jumping), and hyperlinked text; scrollback search (`Ctrl+Shift+F`); Shift+drag for browser text selection instead of tmux copy-mode. **Paste or drop an image** onto a terminal and it's uploaded to `<cwd>/uploads` with the saved path typed at the prompt — made for handing screenshots to Claude Code; ordinary text paste is untouched.
- **Mobile & touch** — swipe to scroll, a fast horizontal flick anywhere to open/close the sidebar, and the layout tracks the on-screen keyboard via `visualViewport` so the prompt and key bar never hide behind it. The **touch key bar** (defaults: Esc, Tab, sticky Ctrl, arrows, `^C`, mic) is fully customizable in Settings → UI: each key has a label, a send sequence in brace-token notation (`{esc}:wq{enter}`, `{^c}` for Ctrl-C, `{{` for a literal brace), and an optional "when" filter that shows it only while a given program (`claude`, `nvim`, …) runs in the pane — reorder keys by drag with a live tag-filterable preview, and render them as the fixed bar below the terminal or a draggable AssistiveTouch-style floating toggle. A 🎤 **voice input** key (Web Speech API, hidden on browsers without it) types dictated text.
...
client/     React + TypeScript — the browser UI; terminal engines (ghostty-web, xterm.js) under src/engines/
