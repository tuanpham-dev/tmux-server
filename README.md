...
- `ctx.registerStatusBarItem({ id, placement?, order?, component })` — a compact readout in the bottom status bar. `placement` is `"left"` or `"right"` (default `"right"`, where the app's own terminal count also sits) and `order` sorts within that group. The component receives a `context` prop with `mobilePointer`, `showMenu(x, y, items)`, and `openPopover(anchorRect, content)` / `closePopover()` — a floating panel above the bar that the host positions and dismisses, and that `openPopover` toggles when the same item calls it again (how the bundled ports extension shows its port list and forward command, and how the external Claude usage extension shows its token counter). The whole bar is hidden on phones and tablets, so an item never renders there
