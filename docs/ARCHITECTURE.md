...
- **worktrees** — two palette commands that open the PROJECTS tree's create-worktree form (`ctx.app.newWorktree`). Worktrees themselves are core: `server/src/gitWorktrees.ts` owns every `git worktree` call, `/api/git/worktrees` serves the tree's middle level, and `client/src/lib/projects.ts`'s `projectTree` groups sessions by repository — an extension can't contribute rows to a core tree. See `plans/worktrees-into-projects.md`
